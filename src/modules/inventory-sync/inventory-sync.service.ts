...
export class InventorySyncService {
  ...
  async getShopSyncStatus(shopId: string) {
    ...
    if (!shop) {
      throw new NotFoundException('Loja não encontrada para sincronização.');
    }
  ...
  private registerShopCron(shop: ShopEntity) {
    ...
    const job = new CronJob(cronExpression, async () => {
      try {
        ...
        this.logger.error(`Falha no cron de sincronização da loja ${shop.name}: ${message}`);
      }
    });
  ...
  private async fetchFeed(feedUrl: string): Promise<ExternalInventoryFeed> {
    ...
    if (!payload?.veiculos || !Array.isArray(payload.veiculos)) {
      throw new Error('Feed de estoque inválido.');
    }
  ...
  private applyExternalVehicle(
    ...
    entity.brand = this.safeString(record.marca);
    entity.model = this.safeString(record.modelo);
    entity.version = this.nullableString(record.versao ?? record.veiculo);
    entity.year = this.parseIntSafe(record.ano, now.getFullYear());
    entity.plate = this.nullableString(record.placa?.toUpperCase());
    entity.color = this.nullableString(record.cor);
    entity.transmission = this.mapTransmission(record.cambio);
    entity.fuelType = this.nullableString(record.combustivel);
    entity.condition = this.nullableString(record.estado);
    entity.categoryType = this.nullableString(record.tipo_categoria);
    entity.mileage = this.parseNullableInt(record.km);
    entity.price = this.resolvePrice(record);
    entity.city = this.nullableString(record.cidade ?? shop.city ?? undefined);
    entity.state = this.nullableString(record.uf ?? shop.state ?? undefined);
  ...
  private buildPhotoCollections(
    ...
    const originalPhotoUrls = (record.fotos ?? []).map((filename) =>
      strategy.buildOriginalUrl(feed, record, filename),
    );
    const thumbnailPhotoUrls = (record.fotos ?? []).map((filename) =>
      strategy.buildThumbnailUrl(feed, record, filename),
    );
