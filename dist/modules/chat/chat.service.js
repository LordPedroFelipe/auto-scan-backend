...
let ChatService = class ChatService {
    constructor(configService, dataSource, shopsRepository, vehiclesRepository, leadsRepository, usersRepository, testDrivesRepository, chatSessionsRepository, chatMessagesRepository, chatTelemetryRepository) {
        ...
        this.aiModel = this.configService.get('OPENAI_MODEL', 'gpt-5-mini') ?? 'gpt-5-mini';
        this.openAiClient = apiKey ? new openai_1.default({ apiKey }) : null;
    }
    ...
    async keywords(user) {
        ...
        const rows = await query.groupBy('keyword').orderBy('count', 'DESC').limit(10).getRawMany();
    ...
    async observability(user) {
        ...
        if (sessionIds.length)
            toolQuery.andWhere('event.sessionId IN (:...sessionIds)', { sessionIds });
        else
            toolQuery.andWhere('1 = 0');
        const toolRows = await toolQuery.groupBy('tool').orderBy('count', 'DESC').limit(10).getRawMany();
        ...
        return {
            ...
            tools: toolRows.filter((row) => !!row.tool).map((row) => ({ name: row.tool, count: Number(row.count) })),
    ...
    async executeConversationTurn(dto) {
        ...
        const recommendations = toolResults.find((tool) => tool.toolName === 'buscarVeiculos')?.payload.vehicles ??
            (await this.findRelevantVehicles(session, intent));
        session.lastRecommendedVehicleIds = recommendations.map((vehicle) => vehicle.id);
        const lead = toolResults.find((tool) => tool.toolName === 'registrarLead')?.payload.lead ?? null;
        if (lead?.id)
    ...
    async buildReply(execution, latestMessage) {
        const deterministic = this.buildDeterministicReply(execution.session, execution.intent, execution.recommendations, execution.lead, latestMessage, execution.contextVehicle, execution.financing, execution.telemetry);
    ...
    async getOrCreateSession(dto) {
        const existing = await this.chatSessionsRepository.findOne({ where: { sessionKey: dto.sessionId } });
        if (existing)
            return this.hydrateSessionState(existing);
        const saved = await this.chatSessionsRepository.save(this.chatSessionsRepository.create({
    ...
    async hydrateSessionState(entity) {
    ...
    normalizeProfileRecord(profile) {
        ...
        return {
            ...
            ...safeProfile,
            preferredCategories: Array.isArray(safeProfile.preferredCategories) ? safeProfile.preferredCategories : [],
            preferredBrands: Array.isArray(safeProfile.preferredBrands) ? safeProfile.preferredBrands : [],
            preferredFuelTypes: Array.isArray(safeProfile.preferredFuelTypes) ? safeProfile.preferredFuelTypes : [],
            preferredTransmissions: Array.isArray(safeProfile.preferredTransmissions) ? safeProfile.preferredTransmissions : [],
            desiredUses: Array.isArray(safeProfile.desiredUses) ? safeProfile.desiredUses : [],
        };
    }
    applyExplicitProfile(session, dto) {
        if (dto.shopId)
            session.shopId = dto.shopId;
        if (dto.vehicleId)
            session.vehicleId = dto.vehicleId;
        if (dto.plate)
            session.customerProfile.plate = dto.plate.toUpperCase();
        if (dto.customerName)
            session.customerProfile.name = dto.customerName.trim();
        if (dto.customerEmail)
            session.customerProfile.email = dto.customerEmail.toLowerCase();
        if (dto.customerPhone)
            session.customerProfile.phone = dto.customerPhone.trim();
        if (dto.customerCity)
            session.customerProfile.city = dto.customerCity.trim();
        this.mergeExtractedProfile(session, dto.message);
    }
    async appendCustomerMessage(session, message) {
    ...
    async appendAssistantMessage(session, message, metadata) {
    ...
    async persistSessionSnapshot(session, overrides) {
        ...
        entity.customerProfile = session.customerProfile;
        ...
        if (overrides?.incrementMessages)
            entity.messagesCount = session.messages.length;
        if (overrides?.lastCustomerMessageAt)
            entity.lastCustomerMessageAt = overrides.lastCustomerMessageAt;
        if (overrides?.lastAssistantMessageAt)
            entity.lastAssistantMessageAt = overrides.lastAssistantMessageAt;
        await this.chatSessionsRepository.save(entity);
    }
    detectIntent(message) {
        const normalized = this.normalize(message);
        if (/(financi|parcel|entrada|prestac|simula)/.test(normalized))
            return 'financing';
        if (/(test drive|dirigir|agendar|agendamento|visita)/.test(normalized))
            return 'test_drive';
        if (/(vendedor|humano|atendente|whatsapp|ligar)/.test(normalized))
            return 'handoff';
        if (/(meu nome|me chama|me chamo|meu telefone|meu whatsapp|meu email|meu e-mail)/.test(normalized))
            return 'lead_capture';
        if (/(placa|esse carro|este carro|esse veiculo|esse veículo)/.test(normalized))
            return 'vehicle_context';
        if (/(carro|suv|sedan|hatch|pickup|caminhonete|economico|econômico|familia|família|ate |até |mil)/.test(normalized))
            return 'vehicle_search';
        return 'greeting';
    }
    async resolveConversationContext(session, dto) {
        ...
        if (session.vehicleId)
            contextVehicle = await this.vehiclesRepository.findOne({ where: { id: session.vehicleId } });
        if (!contextVehicle && session.customerProfile.plate) {
            ...
            if (contextVehicle)
                session.vehicleId = contextVehicle.id;
        }
        if (contextVehicle && !session.shopId)
            session.shopId = contextVehicle.shopId;
        if (!session.shopId && dto.shopId)
            session.shopId = dto.shopId;
        return { contextVehicle, seller: session.shopId ? await this.pickSeller(session.shopId) : null };
    }
    async resolveSelectedVehicleFromMessage(session, message) {
        if (!session.lastRecommendedVehicleIds.length)
            return null;
        ...
        for (const vehicle of candidates) {
            ...
            if (title && normalizedMessage.includes(title))
                return vehicle;
            if (subtitle && normalizedMessage.includes(subtitle))
                return vehicle;
            if (fullLabel && normalizedMessage.includes(fullLabel))
                return vehicle;
        }
        return explicitSelectionSignal ? candidates[0] ?? null : null;
    }
    async executeTools(session, intent, latestMessage, contextVehicle, seller) {
        ...
        try {
            ...
            const names = toolCalls.map((toolCall) => toolCall.name).filter((toolName) => deterministicToolNames.includes(toolName));
    ...
    planToolsDeterministically(session, intent, latestMessage, contextVehicle) {
        const tools = ['buscarVeiculos'];
        const hasUsefulContact = !!(session.customerProfile.name && (session.customerProfile.phone || session.customerProfile.email));
        if (hasUsefulContact && ['handoff', 'financing', 'test_drive', 'lead_capture'].includes(intent))
            tools.push('registrarLead');
        if ((intent === 'handoff' || /(vendedor|whatsapp|humano|ligar)/i.test(latestMessage)) && hasUsefulContact)
            tools.push('transferirParaVendedor');
        if (intent === 'test_drive' && contextVehicle && hasUsefulContact && this.extractScheduleSignal(latestMessage))
            tools.push('agendarTestDrive');
        return Array.from(new Set(tools));
    }
    async executeNamedTools(toolNames, session, intent, latestMessage, contextVehicle, seller) {
        const results = [];
        for (const toolName of toolNames) {
            if (toolName === 'buscarVeiculos')
                results.push({ toolName, payload: { vehicles: await this.findRelevantVehicles(session, intent) } });
            if (toolName === 'registrarLead')
                results.push({ toolName, payload: { lead: await this.registerLeadFromConversation(session, intent, latestMessage) } });
            if (toolName === 'agendarTestDrive')
                results.push({ toolName, payload: { testDrive: await this.scheduleTestDrive(session, latestMessage, contextVehicle) } });
            if (toolName === 'transferirParaVendedor')
                results.push({ toolName, payload: { handoff: await this.transferToSeller(session, seller, latestMessage) } });
    ...
    async findRelevantVehicles(session, intent) {
        ...
        if (session.shopId)
            qb.andWhere('vehicle.shopId = :shopId', { shopId: session.shopId });
        if (session.vehicleId)
            qb.andWhere('vehicle.id = :vehicleId', { vehicleId: session.vehicleId });
        else if (session.customerProfile.plate)
            qb.andWhere('vehicle.plate = :plate', { plate: session.customerProfile.plate });
        else {
            if (session.customerProfile.budgetMax)
                qb.andWhere('vehicle.price <= :maxPrice', { maxPrice: Math.round(session.customerProfile.budgetMax * 1.2) });
            if (session.customerProfile.budgetMin)
                qb.andWhere('vehicle.price >= :minPrice', { minPrice: Math.max(Math.round(session.customerProfile.budgetMin * 0.8), 0) });
            if (session.customerProfile.preferredBrands.length > 0) {
                qb.andWhere(session.customerProfile.preferredBrands.map((_, index) => `vehicle.brand ILIKE :brand${index}`).join(' OR '), Object.fromEntries(session.customerProfile.preferredBrands.map((brand, index) => [`brand${index}`, `%${brand}%`])));
            }
        }
    ...
    rankVehicle(vehicle, session, intent) {
        ...
        if (profile.budgetMin && price >= profile.budgetMin)
            score += 8;
        for (const category of profile.preferredCategories)
            if (searchableText.includes(this.normalize(category))) {
                score += 14;
                reasons.push(`combina com o perfil ${category}`);
            }
        for (const brand of profile.preferredBrands)
            if (this.normalize(vehicle.brand).includes(this.normalize(brand))) {
                score += 12;
                reasons.push(`marca alinhada com a preferencia por ${brand}`);
            }
        for (const fuel of profile.preferredFuelTypes)
            if (searchableText.includes(this.normalize(fuel))) {
                score += 10;
                reasons.push(`motorizacao compativel com preferencia por ${fuel}`);
            }
        for (const transmission of profile.preferredTransmissions)
            if (searchableText.includes(this.normalize(transmission))) {
                score += 9;
                reasons.push(`cambio ${transmission.toLowerCase()} como voce pediu`);
            }
        if (profile.desiredUses.includes('cidade') && (vehicle.mileage ?? 0) < 80000) {
            score += 6;
            reasons.push('boa opcao para uso urbano');
        }
        if (profile.desiredUses.includes('familia') && /(suv|sedan|7 lugares)/.test(searchableText)) {
            score += 9;
            reasons.push('tem perfil forte para uso em familia');
        }
        if (profile.desiredUses.includes('economia') && /(flex|hibrido|1\.0|1\.3)/.test(searchableText)) {
            score += 8;
            reasons.push('boa chance de custo de uso equilibrado');
        }
        if ((vehicle.mileage ?? 0) <= 40000) {
            score += 6;
            financeScore += 4;
            reasons.push('quilometragem competitiva');
        }
        if (vehicle.isOnOffer) {
            score += 5;
            financeScore += 5;
            reasons.push('esta em oferta');
        }
        if (vehicle.isHighlighted) {
            score += 5;
            reasons.push('e um destaque da loja');
        }
        if (intent === 'financing') {
            financeScore += 15;
            if (priceDelta !== null && priceDelta < 0) {
                financeScore += 10;
                reasons.push('faz sentido para avancar com financiamento');
            }
        }
        return {
            id: vehicle.id,
    ...
    async registerLeadFromConversation(session, intent, latestMessage) {
        const profile = session.customerProfile;
        const hasUsefulContact = !!(profile.name && (profile.phone || profile.email));
        if (!hasUsefulContact || !session.shopId)
            return null;
        ...
        if (session.leadId) {
            ...
            if (existingLead) {
                ...
                if (['handoff', 'test_drive', 'financing'].includes(intent))
                    existingLead.status = lead_entity_1.LeadStatus.Qualified;
        ...
        const createdLead = await this.leadsRepository.save(this.leadsRepository.create({
            ...
            origin: 'Chat IA',
            notes,
            status: ['handoff', 'test_drive', 'financing'].includes(intent) ? lead_entity_1.LeadStatus.Qualified : lead_entity_1.LeadStatus.New,
    ...
    async scheduleTestDrive(session, latestMessage, contextVehicle) {
        const profile = session.customerProfile;
        if (!contextVehicle || !profile.name || !(profile.phone || profile.email))
            return null;
        ...
        return this.testDrivesRepository.save(this.testDrivesRepository.create({
            ...
            status: test_drive_entity_1.TestDriveStatus.Pending,
        }));
    }
    async transferToSeller(session, seller, latestMessage) {
    ...
    buildLeadNotes(session, latestMessage) {
        const profile = session.customerProfile;
        const tags = [
    ...
    buildDeterministicReply(session, intent, recommendations, lead, latestMessage, contextVehicle, financing, telemetry) {
        const profile = session.customerProfile;
        const customerName = profile.name ? `, ${profile.name}` : '';
        const options = this.buildQuickOptions(intent, lead, recommendations, financing);
        const photos = recommendations.map((vehicle) => vehicle.photoUrl).filter((url) => !!url);
        ...
        if (contextVehicle)
            message += `Ja estou considerando o ${contextVehicle.brand} ${contextVehicle.model}${contextVehicle.version ? ` ${contextVehicle.version}` : ''} como contexto principal. `;
        if (intent === 'financing')
            message += 'Montei uma visao comercial para avancarmos no financiamento. ';
        else if (intent === 'test_drive')
            message += 'Perfeito, ja organizei o caminho para um test drive com menos atrito. ';
        else if (intent === 'handoff')
            message += 'Faz sentido envolver um vendedor humano agora para acelerar a conversao. ';
        else if (intent === 'vehicle_context')
            message += 'Analisei o veiculo em contexto e comparei com alternativas do estoque. ';
        else if (intent === 'vehicle_search')
            message += 'Cruzei o que voce pediu com o estoque real da loja. ';
        else
            message += 'Posso atuar como seu vendedor digital e conduzir a proxima melhor acao. ';
        if (recommendations.length > 0) {
            ...
            if (top.reasons.length > 0)
                message += `Ele apareceu forte porque ${top.reasons.join(', ')}. `;
            if (recommendations.length > 1)
                message += `Tambem deixei mais ${recommendations.length - 1} opcao(oes) bem aderentes na manga. `;
        }
        else {
        ...
        if (financing)
            message += `Para financiamento, uma entrada estimada de R$ ${financing.downPayment.toLocaleString('pt-BR')} deixaria ${financing.termMonths}x de cerca de R$ ${financing.estimatedInstallment.toLocaleString('pt-BR')}. `;
        if (lead?.sellerName)
            message += `Ja deixei a conversa preparada para seguir com ${lead.sellerName}. `;
        else if (shouldCaptureLead)
            message += 'Se fizer sentido, ja posso registrar seu nome e telefone para o vendedor continuar exatamente deste ponto. ';
        if (telemetry.toolsUsed.length > 0)
            message += `Nesta etapa eu usei ${telemetry.toolsUsed.join(', ')} para nao responder no escuro.`;
        return { id: (0, crypto_1.randomUUID)(), message: message.trim(), options, photos, humor: recommendations.length > 0 ? 'happy' : 'curious', vehicles: recommendations, lead, profile, handoffSuggested, shouldCaptureLead, nextBestAction, summary: session.summary, financing, telemetry };
    }
    buildQuickOptions(intent, hasLead, vehicles, financing) {
        ...
        if (intent !== 'financing')
            options.set('financiamento', { label: 'Simular financiamento', kind: 'prompt', action: 'simular_financiamento' });
        if (intent !== 'test_drive')
            options.set('test_drive', { label: 'Agendar test drive', kind: 'prompt', action: 'agendar_test_drive' });
        if (!hasLead) {
        ...
        if (financing)
            options.set('entrada', { label: 'Quero ajustar a entrada', kind: 'prompt', action: 'ajustar_entrada' });
    ...
    async enrichWithOpenAi(execution, deterministic, latestMessage) {
        if (!this.openAiClient)
    ...
    buildFinancingSimulation(recommendations, latestMessage) {
        const targetVehicle = recommendations[0];
        if (!targetVehicle || !/(financi|parcel|entrada|prestac|simula)/i.test(latestMessage))
            return null;
    ...
    extractDownPayment(message, price) {
        const normalized = this.normalize(message);
        const percentageMatch = normalized.match(/(\d{1,2})\s?%\s?(?:de entrada|entrada)/i);
        if (percentageMatch)
            return Math.round(price * (Number(percentageMatch[1]) / 100));
        const valueMatch = normalized.match(/entrada\s*(?:de\s*)?(?:r\$\s*)?(\d+[\d\.,]*)/i);
        if (valueMatch)
            return this.parseMoneyToken(valueMatch[1], false);
        return null;
    }
    extractTermMonths(message) {
        const match = this.normalize(message).match(/(\d{1,3})\s?x\b/);
        return match ? Number(match[1]) : null;
    }
    extractScheduleSignal(message) {
        const normalized = this.normalize(message);
        const now = new Date();
        if (/amanha|amanhã/.test(normalized))
            return { preferredDate: new Date(now.getTime() + 24 * 60 * 60 * 1000), preferredTime: this.extractTimeToken(message) };
        if (/hoje/.test(normalized))
            return { preferredDate: now, preferredTime: this.extractTimeToken(message) };
    ...
    async streamReply(sessionId, reply, telemetry) {
        ...
        for (let index = 0; index < chunks.length; index += 1)
            this.publishStream(sessionId, 'reply.chunk', { replyId: reply.id, index, content: chunks[index] });
        this.publishStream(sessionId, 'reply.completed', reply);
    }
    getStreamChannel(sessionId) {
        if (!this.streamChannels.has(sessionId))
            this.streamChannels.set(sessionId, new rxjs_1.Subject());
        return this.streamChannels.get(sessionId);
    }
    publishStream(sessionId, type, data) {
    ...
    ensureSessionAccess(session, user) {
        if (!user.shopId)
            return;
        if (session.shopId !== user.shopId) {
            throw new common_1.ForbiddenException('Sessao fora do escopo da loja do usuario.');
        }
    }
    async logTelemetry(sessionId, type, message, payload, level = 'info') {
    ...
    mergeExtractedProfile(session, message) {
        ...
        if (plateMatch)
            profile.plate = plateMatch[0].toUpperCase();
        const budget = this.extractBudgetRange(message);
        if (budget.max)
            profile.budgetMax = budget.max;
        if (budget.min)
            profile.budgetMin = budget.min;
    ...
    extractBudgetRange(message) {
        const normalized = this.normalize(message);
        const results = { min: null, max: null };
        const maxMatch = normalized.match(/(?:ate|até)\s*(?:r\$)?\s*(\d+[\d\.,]*)(?:\s*mil)?/i);
        if (maxMatch)
            results.max = this.parseMoneyToken(maxMatch[1], maxMatch[0].includes('mil'));
        ...
        if (!results.max) {
            ...
            if (singleBudget)
                results.max = Number(singleBudget[1]) * 1000;
        }
        return results;
    }
    parseMoneyToken(token, hasMilWord) {
        const digits = token.replace(/\./g, '').replace(',', '.');
        const numeric = Number(digits);
        if (!Number.isFinite(numeric))
            return null;
        if (hasMilWord || numeric < 1000)
            return numeric * 1000;
        return numeric;
    }
    extractCategories(normalized) {
    ...
    extractTransmissions(normalized) {
        const transmissions = [
    ...
    mergeUnique(current, incoming) {
        return Array.from(new Set([...current, ...incoming.filter((item) => !!item)]));
    }
    toKeyword(value) { return value.trim(); }
    toTitleCase(value) { return value.toLowerCase().split(/\s+/).map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(' '); }
};
...
exports.ChatService = ChatService = __decorate([
    ...
    __param(7, (0, typeorm_1.InjectRepository)(chat_session_entity_1.ChatSessionEntity)),
    __param(8, (0, typeorm_1.InjectRepository)(chat_message_entity_1.ChatMessageEntity)),
    __param(9, (0, typeorm_1.InjectRepository)(chat_telemetry_event_entity_1.ChatTelemetryEventEntity)),
